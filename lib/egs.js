(function (GLOBAL) {
  "use strict";
  var __create, __import, __isArray, __lte, __num, __once, __owns, __slice, __strnum, __toArray, __typeof, buildFileCache, compileCache, fileCache, findFileCache, fs, gorillascript, helpers, PARTIAL_PREFIX, path, TextBuilder, utils;
  ...
  function readFile(path, shouldCache, callback) {
    ...
    if (shouldCache) {
      if (__owns.call(fileCache, path)) {
        ...
        return _ref(__once(function (_e, result) {
          if (_e != null) {
            return callback(_e);
          }
          return callback(null, fileCache[path] = result);
        }));
      }
    } else {
  ...
  function findFile(name, fromFilename, shouldCache, callback) {
    ...
    if (shouldCache) {
      if (__owns.call(findFileCache, _ref2 = __strnum(name) + "\u0000" + __strnum(fromFilename))) {
        return callback(null, findFileCache[_ref2]);
      } else {
        return _ref(__once(function (_e, result) {
          var _ref;
          if (_e != null) {
          ...
          return callback(null, findFileCache[_ref] = result);
        }));
      }
    } else {
      return _ref(callback);
  ...
  function findAndGetCompiled(name, fromFilename, options, callback) {
    return findFile(name, fromFilename, options.cache, __once(function (_e, filename) {
      if (_e != null) {
        return callback(_e);
      }
      return readFile(filename, options.cache, __once(function (_e2, text) {
        ...
        return getCompiled(text, filename, options, __once(function (_e3, compiled) {
          if (_e3 != null) {
            return callback(_e3);
          }
          return callback(null, filename, compiled);
        }));
      }));
    }));
  }
  ...
  function getStandardHelpers(options, context, callback) {
    var _ref, blocks, blockWriters, blockWritersDepth, depthKey, errored, nameKey, pending;
    blocks = {};
    blockWriters = {};
    blockWritersDepth = {};
    pending = 1;
    ...
    _ref.partial = function (name, write, locals) {
      ...
      if (typeof options.partialPrefix === "string") {
        ...
        partialPrefix = PARTIAL_PREFIX;
      }
      name = partialPrefix + name;
      pend();
      findAndGetCompiled(name, this[nameKey], options, __once(function (_e, filename, func) {
        var _ref;
        if (_e != null) {
          throw _e;
        }
        try {
          func(write, ((_ref = __import(__create(context), locals))[nameKey] = filename, _ref));
        } catch (e) {
          errored = true;
          return callback(e);
        }
    ...
    _ref["extends"] = function (name, write, locals) {
      ...
      findAndGetCompiled(name, this[nameKey], options, __once(function (_e, filename, func) {
        ...
        try {
          func(write, ((_ref = __import(__create(context), locals))[depthKey] = __num(_this[depthKey]) + 1, _ref[nameKey] = filename, _ref));
        } catch (e) {
          errored = true;
          return callback(e);
        }
  ...
  function getCompiled(text, key, options, callback) {
    ...
    if (options.cache) {
      if (__owns.call(compileCache, key)) {
        callback(null, compileCache[key]);
      } else {
        _ref(__once(function (_e, result) {
          if (_e != null) {
            return callback(_e);
          }
          return callback(null, compileCache[key] = result);
        }));
      }
    } else {
      _ref(callback);
  ...
  function build(text, options, callback) {
    ...
    function render(data, callback) {
      ...
      return fetch(__once(function (_e, func) {
        ...
        context = makeContext(options, data, __once(function (e) {
          if (e != null) {
            return callback(e);
          } else {
  ...
  function buildFile(path, options, callback) {
    ...
    function getBuilder(cb) {
      return readFile(path, options.cache, __once(function (_e, text) {
        if (_e != null) {
          return cb(_e);
        }
        return build(text, options, cb);
      }));
    }
    if (options.cache) {
      if (__owns.call(buildFileCache, _ref = makeCacheKey(options))) {
        callback(null, buildFileCache[_ref]);
      } else {
        getBuilder(__once(function (_e, result) {
          var _ref;
          if (_e != null) {
            return callback(_e);
          }
          return callback(null, buildFileCache[_ref] = result);
        }));
      }
    } else {
      getBuilder(callback);
