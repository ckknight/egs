(function (GLOBAL) {
  ...
  findAndCompileFile = __promise(function (name, fromFilepath, compileOptions) {
      ...
      while (true) {
        ...
        case 0:
          ...
          return {
            ...
            value: compileFile(filepath, compileOptions)
          };
        case 1:
          compiled = _received;
          ++_state;
  ...
  function makeStandardHelpers(options) {
    ...
    if (options[packageKey]) {
      ...
      fetchCompiled = function (context, name) {
        return findAndCompileFile(name, context[filepathKey], getCompileOptions(options));
      };
    }
  ...
  function compileTemplate(egsCode, options) {
    ...
    return makeTemplate(
      compile(egsCode, getCompileOptions(options)),
      options
    );
  }
  function compileTemplateFromFile(filepath, options) {
    ...
    return makeTemplate(
      compileFile(filepath, getCompileOptions(options)),
      options
    );
  }
