(function (GLOBAL) {
  ...
  jQuery(function ($) {
    var currentTemplate, handleTry, hasTouch, inToc, inTocLabel, sideBySide;
    ...
    function updateResult() {
      ...
      return __fromPromise(GorillaScript["eval"](text))((_once = false, function (err, data) {
        ...
        if (err) {
          ...
          return __fromPromise(currentTemplate(data))((_once2 = false, function (err, result) {
            if (_once2) {
              throw Error("Attempted to call function more than once");
            } else {
              _once2 = true;
            ...
            if (err) {
              $("#try-input-data-wrap").addClass("error");
              return $("#try-output-result").val("// Error rendering template: " + String(err));
            } else {
              $("#try-input-data-wrap").removeClass("error");
              return $("#try-output-result").val(result);
            }
          }));
        }
      }));
