(function (GLOBAL) {
  ...
  jQuery(function ($) {
    var currentStream, currentTemplate, handleTry, hasTouch, inToc, inTocLabel,
        sideBySide;
    ...
    handleTry = (function () {
      ...
      function handle() {
        ...
        text = $("#try-input-template").val();
        currentTemplate = EGS(text);
        compiling = true;
        return __fromPromise(EGS.compile(text))((_once = false, function (err, result) {
          if (_once) {
            throw Error("Attempted to call function more than once");
          } else {
            _once = true;
          ...
          compiling = false;
          if (err) {
            $("#try-input-template-wrap").addClass("error");
            $("#try-output-template").val("// Error compiling template: " + String(err));
          } else {
            $("#try-input-template-wrap").removeClass("error");
            $("#try-output-template").val(result.code);
          }
          return setTimeout(updateResult, 10);
        }));
      }
      return function () {
        if (interval != null) {
    ...
    function updateResult() {
      var _once, text;
      text = $("#try-input-data").val();
      return __fromPromise(GorillaScript["eval"](text))((_once = false, function (err, data) {
        var buffer, stream;
        if (_once) {
          throw Error("Attempted to call function more than once");
        } else {
        ...
        if (err) {
          $("#try-input-data-wrap").addClass("error");
          return $("#try-output-result").val("// Error reading data: " + String(err));
        } else if (currentTemplate) {
          buffer = [];
          return stream = currentStream = currentTemplate.stream(data).on("data", function (text) {
            if (currentStream === stream) {
              $("#try-input-data-wrap").removeClass("error");
              buffer.push(text);
              return $("#try-output-result").val(buffer.join(""));
            }
          }).on("error", function (err) {
            if (currentStream === stream) {
              $("#try-input-data-wrap").addClass("error");
              $("#try-output-result").val(String(err != null && err.stack || stack));
              return currentStream = void 0;
            }
          }).on("done", function () {
            if (currentStream === stream) {
              return currentStream = void 0;
    ...
    setInterval(
      (function () {
        ...
        return function () {
          ...
          if (text !== lastText) {
            lastText = text;
            return setTimeout(updateResult, 250);
          }
        };
      }()),
      17
