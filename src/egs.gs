...
let compile = promise! #(egs-code as String, compile-options as {}, helper-names as [])* as Promise<Function<Promise, Function, {}>>
  ...
  let options = {} <<< compile-options <<< {+embedded, +noindent, macros, prelude: null, ast-pipe: get-ast-pipe(helper-names)}
...
let simple-helpers-proto = {extends GLOBAL} <<< helpers
let helpers-proto = {extends GLOBAL} <<< helpers <<< {
